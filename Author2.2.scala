...
class Book(var n: String, var a: List[Author], var p: Double, var q: Int){

    def this(n: String, a: List[Author], p: Double) = this(n, a, p, 0)

    private var _name: String = n
    private var _authors: List[Author] = a
    private var _price: Double = p
    private var _quantity: Int = q

    def name: String = this._name

    def authors: List[Author] = this._authors

    def authorNames: String = authors.mkString(", ")
    ...
    override def toString(): String = {
      "Book[name= " + name + "," + authors.toString() + ", price= " + price + ", quantity= " + quantity
    }
